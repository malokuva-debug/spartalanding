...
import type { SalonData, Service, Worker } from "@/lib/turso";
...
export default function BookingForm({ lang, salon, loading, preselect, onConsumed }: Props) {
  ...
  useEffect(() => {
    if (date && serviceId) {
      setTime("");
      load(date, serviceId, workerId);
    }
  }, [date, serviceId, workerId, load]);
          ...
          {workers.length > 0 && (
            <div className="mb-6">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1.5 scrollbar-thin">
                ...
                >
                  <span className={`w-8 h-8 rounded-lg grid place-items-center ${workerId === "any" ? "bg-white/15" : "bg-brand-50 text-brand-600"}`}>
                    <Users className="w-3.5 h-3.5" />
                  </span>
                  <span className="text-left">
                    <span className="block text-[12px] font-semibold leading-tight">{c.anyone}</span>
                    <span className={`block text-[10px] leading-tight ${workerId === "any" ? "text-white/60" : "text-slate-400"}`}>
                      {c.anyoneHint}
                    </span>
                  </span>
                </button>
                    ...
                    >
                      <span
                        className={`w-8 h-8 rounded-lg grid place-items-center text-[11px] font-bold ${
                          on ? "bg-white/15 text-white" : off ? "bg-slate-100 text-slate-300" : "bg-gold-100 text-gold-700"
                        }`}
                      >
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between gap-3 pt-2 border-t border-white/10">
                <span className="inline-flex items-center gap-2 text-white/45 text-xs">
                  <Users className="w-3.5 h-3.5" />
                  {workerId === "any" ? c.anyone : workers.find((w) => w.id === workerId)?.name}
